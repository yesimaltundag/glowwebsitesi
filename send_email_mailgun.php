...
// Mailgun ile e-posta gönderme
function sendEmailMailgun($to, $subject, $message, $from_name = "GLOW Sitesi") {
    // Mailgun API Key - Ücretsiz hesap için
    $api_key = 'key-YOUR_API_KEY'; // Mailgun API Key buraya
    $domain = 'your-domain.mailgun.org'; // Mailgun domain
    ...
    // Mailgun API isteği
    $data = array(
        'from' => $from_name . ' <noreply@' . $domain . '>',
        'to' => $to,
        'subject' => $subject,
        'html' => $html_message
    );
    
    // cURL ile Mailgun API'ye gönder
    $ch = curl_init();
    ...
    return $http_code === 200; // 200 = Başarılı
}
